...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *buf, size_t len) {
  ...
  // set, so it costs nothing during real fuzzing. Bypasses the selector and
  // calls parseJSONCounter() directly on the payload, printing its actual
  // return code (0 = valid JSON it could tokenize, -1/255 = parse failure) —
  // a neutered/no-op parser can't reproduce both values on demand.
  if (getenv("MAYHEM_KAT_PROBE") != NULL) {
    int rc = (int8_t)zmq_iface->parseJSONCounter(json, (int)payload_len);
    fprintf(stdout, "MAYHEM_KAT parseJSONCounter_rc=%d\n", rc);
    fflush(stdout);
    free(json);
    return 0;
  }
