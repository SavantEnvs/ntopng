...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *buf, size_t len) {
  ...
  // KAT probe (mayhem/test.sh oracle): only active when MAYHEM_KAT_PROBE is
  // set, so it costs nothing during real fuzzing. Prints the nDPI protocol
  // ntopng's dissector actually computed for the flow plus the packet count,
  // so a neutered/no-op dissector (which never populates `flow`) is caught.
  if (getenv("MAYHEM_KAT_PROBE") != NULL) {
    int master_proto = -1, app_proto = -1;
    if (flow != NULL) {
      ndpi_protocol p = flow->get_detected_protocol();
      master_proto = (int)p.proto.master_protocol;
      app_proto = (int)p.proto.app_protocol;
    }
    fprintf(stdout, "MAYHEM_KAT master=%d app=%d packets=%llu\n", master_proto,
	    app_proto, (unsigned long long)iface->getNumPackets());
    fflush(stdout);
  }
